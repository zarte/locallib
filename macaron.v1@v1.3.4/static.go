...
type StaticOptions struct {
	...
	//banfiles  these files can't be accessed
	Banfiles []string
}
...
func staticHandler(ctx *Context, log *log.Logger, opt StaticOptions) bool {
	...
	//采用字符串匹配较为高效,如需精确匹配自行修改
	if len(opt.Banfiles)>0 {
		for _,v  := range opt.Banfiles {
			if strings.Index(file,v) >0 {
				fmt.Println("is ban:",file,time.Now())
				return false
			}
		}
	}
